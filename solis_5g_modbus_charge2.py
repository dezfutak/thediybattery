...
mqtt_client         = "homeassistant" #Change this line accordingly
mqtt_server         = "localhost" #Change this line accordingly
mqtt_username       = "mqttuser" #Change this line accordingly
mqtt_password       = "XXXXXXXX" #Change this line accordingly
debug               = False
...
def get_status():
    ...
    #been run, or not 
    with open("/home/pi/code/battery/writetoinverter", "r") as file: # Change this line accordingly
    ...
    #Grid Voltage
    grid_voltage = instrument.read_register(33128, functioncode=4, signed=False)
    grid_voltage = grid_voltage/10
    ...
    battery_max_discharge_current = instrument.read_long(43118, functioncode=4, signed=False)
    battery_max_discharge_current = battery_max_discharge_current/10
    ...
    #Total kWh since commissioning
    totalkwh = instrument.read_register(33030, functioncode=4, signed=False)

    #Total generation this year (kWh)
    thisyearkwh = instrument.read_register(33038, functioncode=4, number_of_decimals=1, signed=False)
    ...
    with open("/home/pi/code/battery/chargehourstart", "r") as file: # Change this line accordingly
        chargehourstart = file.read()
        chargehourstart = int(chargehourstart.rstrip('\n'))
    #    print(str(chargehourstart))
    with open("/home/pi/code/battery/chargeminutestart", "r") as file: # Change this line accordingly
    ...
    msgs.append((mqtt_topic + "battery_max_discharge_current", battery_max_discharge_current, 0, False))
    ...
    msgs.append((mqtt_topic + "generation_alltime", totalkwh, 0, False))
    msgs.append((mqtt_topic + "generation_thisyear", thisyearkwh, 0, False))
    ...
    msgs.append((mqtt_topic + "grid_power", grid_power, 0, False))
    msgs.append((mqtt_topic + "grid_voltage", grid_voltage, 0, False))
...
while True:
    ...
    if debug:
        print("-------------------------------")
    time.sleep(1) # Change this line accordingly. Larger values mean less frequent reading from the Solis COM output
